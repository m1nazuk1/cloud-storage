...
/** Категории без «все» — для мультивыбора в группах */
...
/** Категория файла для фильтра (одна основная). */
export function getFileCategory(file: FileMetadata): Exclude<FileCategoryFilter, 'all'> {
    const ext = extOf(file);
    const mime = (file.mimeType || '').toLowerCase();

    if (mime.startsWith('image/') || IMG_EXT.has(ext)) return 'images';
    if (mime.startsWith('audio/') || mime.startsWith('video/')) return 'media';
    if (MEDIA_EXT.has(ext)) return 'media';
    if (ARCH_EXT.has(ext) || mime.includes('zip') || mime.includes('compressed')) return 'archives';
    if (CODE_EXT.has(ext) || mime.includes('javascript') || mime.includes('json')) return 'code';
    if (
        DOC_EXT.has(ext) ||
        mime.includes('pdf') ||
        mime.includes('word') ||
...
/** Пустой набор категорий = без ограничения по типу (показать все типы). */
...
/** Пустой набор id = без ограничения по автору. Файл без uploader.id не попадает в выборку, если фильтр включён. */
export function fileMatchesUploaderSet(file: FileMetadata, uploaderIds: Set<string>): boolean {
    if (uploaderIds.size === 0) return true;
    const uid = file.uploader?.id;
    if (uid === undefined || uid === null || uid === '') return false;
    return uploaderIds.has(String(uid));
}

export function filterGroupFiles(
    files: FileMetadata[],
    categorySet: Set<NonAllCategory>,
    uploaderIds: Set<string>,
    search: string
): FileMetadata[] {
    return files.filter(
        (f) =>
            fileMatchesCategorySet(f, categorySet) &&
