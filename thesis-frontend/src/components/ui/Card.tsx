...
const Card: React.FC<CardProps> = ({ children, className = '', hover = true, style }) => {
    return (
        <div
            style={style}
            className={`rounded-2xl border border-slate-200/80 dark:border-slate-700/90 bg-white/90 dark:bg-slate-800/85 backdrop-blur-sm shadow-lg shadow-slate-900/5 dark:shadow-black/30 ${
                hover
...
export const CardHeader: React.FC<CardHeaderProps> = ({ children, className = '' }) => {
    return (
        <div className={`px-4 py-3 sm:px-6 sm:py-4 border-b border-slate-100/90 dark:border-slate-700/80 ${className}`}>
            {children}
        </div>
    );
};
