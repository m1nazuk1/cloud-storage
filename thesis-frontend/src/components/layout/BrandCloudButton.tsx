...
/** Кубическая кривая Безье от p0 к p3 с контрольными p1, p2. t ∈ [0,1]. */
...
/**
 * Строит плавную траекторию: разгон к большой орбите вокруг центра экрана → полный круг → широкий возврат домой.
 */
function buildFlightKeyframes(fly: { x: number; y: number }): Keyframe[] {
    ...
    // Радиус орбиты: крупный, но строго в пределах safe-area от краёв
    ...
    // Широкая «дуга» домой: нижняя часть экрана и левый верх — выглядит как плавный занос
...
/**
 * Клик по облаку — плавный полёт по экрану (WAAPI, плотные keyframes), петля в центре, возврат на место.
 */
const BrandCloudButton: React.FC<{
    ...
    useEffect(() => {
        if (!fly || !motionRef.current) return undefined;
