...
const Sidebar: React.FC<SidebarProps> = ({ mobileOpen = false, onMobileClose, collapsed: collapsedProp, onCollapsedChange, }) => {
    ...
    const navItems = [
        ...
        { to: '/groups', icon: Folder, label: 'Группы' },
        { to: '/notifications', icon: Bell, label: 'Уведомления' },
        { to: '/profile', icon: User, label: 'Профиль' },
    ];
    const linkClass = ({ isActive }: {
        isActive: boolean;
    }) => `group flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 active:scale-[0.98] ${isActive
    ...
    return (<>
            {mobileOpen && (<div className="lg:hidden fixed inset-0 z-50 flex">
                    <button type="button" className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm cursor-default" aria-label="Закрыть меню" onClick={closeMobile}/>
                    <div className="relative flex flex-col max-w-[min(100vw-2rem,20rem)] w-full max-h-[100dvh] bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 shadow-2xl shadow-indigo-950/50 border-r border-white/10">
                        <div className="flex-1 h-0 pt-6 pb-4 overflow-y-auto">
                            <div className="flex items-center flex-shrink-0 px-4">
                                <BrandCloudButton className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-400 to-violet-500 shadow-lg shadow-indigo-500/30 border-0 p-0 cursor-pointer hover:brightness-110 active:scale-[0.97] transition-[filter,transform] duration-200"/>
            ...
            <div className={`hidden lg:fixed lg:inset-y-0 lg:z-40 lg:flex lg:flex-col transition-[width] duration-300 ease-out ${collapsed ? 'lg:w-20' : 'lg:w-64'}`}>
                <div className="flex flex-col flex-grow bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 border-r border-white/10 pt-6 pb-4 overflow-y-auto shadow-xl shadow-indigo-950/30">
                    <div className="flex items-center flex-shrink-0 px-4">
                        <BrandCloudButton className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-400 to-violet-500 shadow-lg shadow-indigo-500/25 border-0 p-0 cursor-pointer hover:brightness-110 active:scale-[0.97] transition-[filter,transform] duration-200"/>
                        {!collapsed && (<span className="ml-3 text-xl font-bold bg-gradient-to-r from-white to-indigo-200 bg-clip-text text-transparent">
                                Облако
                            </span>)}
                        <button type="button" onClick={() => setCollapsed(!collapsed)} className="ml-auto p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 focus:outline-none transition-colors" aria-label={collapsed ? 'Развернуть меню' : 'Свернуть меню'}>
                    ...
                    <div className="flex-shrink-0 border-t border-white/10 p-4 mt-auto">
                        <div className="flex items-center w-full gap-2">
                            ...
                            <button type="button" onClick={handleLogout} className="shrink-0 p-2 rounded-lg text-slate-400 hover:text-rose-300 hover:bg-white/10 transition-colors" title="Выйти" aria-label="Выйти">
