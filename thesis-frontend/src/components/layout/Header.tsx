...
const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
    ...
    return (
        <header className="sticky top-0 z-30 w-full border-b border-white/50 dark:border-slate-700/70 bg-white/80 dark:bg-slate-900/90 backdrop-blur-xl shadow-sm shadow-indigo-950/5 dark:shadow-black/25 transition-colors duration-300 pt-safe">
            <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 w-full min-w-0">
                {/* Мобильная шапка: меню | действия; поиск ниже на всю ширину */}
                <div className="flex flex-col gap-2 py-2 sm:py-0 sm:h-16 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                    <div className="flex items-center justify-between gap-2 min-h-[2.75rem] sm:min-h-0 sm:flex-1 sm:min-w-0">
                        <div className="flex items-center gap-2 min-w-0 flex-1 sm:flex-initial">
                            <button
                                type="button"
                                className="lg:hidden shrink-0 p-2 -ml-1 text-slate-500 dark:text-slate-400 hover:text-indigo-700 dark:hover:text-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-400 rounded-xl transition-colors touch-manipulation"
                                onClick={onMenuClick}
