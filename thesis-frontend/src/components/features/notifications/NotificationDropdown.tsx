...
const DROPDOWN_FILTERS: { id: NotificationGroupFilter; short: string }[] = [
    { id: 'all', short: 'Все' },
...
const NotificationDropdown: React.FC = () => {
    ...
    const filteredNotifications = useMemo(
        () => filterNotifications(notifications, notifFilter),
    ...
    return (
        <div className="relative">
            <button
                ...
                className="relative rounded-xl p-2 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-indigo-200 focus:outline-none focus:ring-2 focus:ring-indigo-400/60 transition-all duration-200 hover:scale-105 active:scale-95"
                aria-expanded={isOpen}
                aria-haspopup="true"
            >
                <Bell className="h-6 w-6" />
                {(unreadCount?.count ?? 0) > 0 && (
                    <span className="absolute top-1 right-1 block h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-900 shadow-sm" />
                )}
            </button>
            ...
            {isOpen && (
                <>
                    ...
                    <div
                        className={
                            'fixed z-50 left-2 right-2 top-[4.75rem] max-h-[min(78vh,32rem)] flex flex-col rounded-2xl border border-white/60 dark:border-slate-600 bg-white/95 dark:bg-slate-900/95 shadow-2xl shadow-indigo-950/20 dark:shadow-black/40 backdrop-blur-xl ring-1 ring-indigo-950/5 dark:ring-slate-700 overflow-hidden ' +
                    ...
                    >
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div className="min-w-0 flex-1">
                                    ...
                                    {unreadCount != null && unreadCount.count > 0 && (
                                        <span className="mt-1 inline-flex items-center rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-medium text-indigo-800 dark:bg-indigo-900/55 dark:text-indigo-200">
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                {!notification.read && (
                                                    <button
                                                        ...
                                                        className="shrink-0 h-9 w-9 flex items-center justify-center rounded-lg text-slate-400 hover:bg-white hover:text-indigo-600 hover:shadow-sm border border-transparent hover:border-slate-200 dark:hover:bg-slate-800 dark:hover:text-indigo-300 dark:hover:border-slate-600 transition-all"
                        ...
                        <div className="shrink-0 p-4 pb-safe border-t border-slate-100 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/80 space-y-2">
                            <Link
                                ...
                                className="block w-full text-center text-sm font-semibold text-indigo-600 hover:text-violet-700 dark:text-indigo-400 dark:hover:text-violet-300 transition-colors"
                            >
                                Все уведомления
                            </Link>
                            <button
                                type="button"
                                onClick={() => setIsOpen(false)}
                                className="w-full text-center text-sm text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 py-1"
                            >
                                Закрыть
                            </button>
                        </div>
