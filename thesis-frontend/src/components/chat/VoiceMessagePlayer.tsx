...
const VoiceMessagePlayer: React.FC<Props> = ({ fileId, mimeType, mine }) => {
    ...
    if (!src) {
        return (<div className="flex items-center gap-2 py-1 text-xs opacity-70">
    ...
    return (<div className="flex items-center gap-2.5 min-w-0 max-w-[min(100%,320px)] py-0.5">
            ...
            const px = 4 + h * 26;
            return (<div key={i} className={`min-w-[2px] max-w-[3px] flex-1 rounded-full transition-colors duration-75 ${played ? barPlayed : barBase}`} style={{ height: `${px}px` }}/>);
        })}
                </div>
                <div className={`flex justify-between gap-2 text-[11px] tabular-nums font-medium tracking-tight ${mine ? 'text-white/85' : 'text-slate-500 dark:text-slate-400'}`}>
