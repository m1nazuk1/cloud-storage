...
function mentionMatch(text: string): { start: number; filter: string } | null {
    const re = /(^|\s)@([\w.]*)$/;
...
function renderTextWithMentions(text: string, members: User[], mine: boolean): React.ReactNode {
    ...
    return parts.map((part, i) => {
        if (part.startsWith('@')) {
            ...
            return (
                <span key={i} className={isMember ? mentionClass : undefined}>
                    {part}
                </span>
            );
        }
...
const GroupChat: React.FC = () => {
    ...
    const updateActionMenuPosition = useCallback((btn: HTMLElement) => {
        ...
        let top =
            spaceBelow >= MENU_H || spaceBelow >= spaceAbove ? r.bottom + GAP : r.top - MENU_H - GAP;
    ...
    useLayoutEffect(() => {
        if (!openActionMenuId || !lastMsgMenuButtonRef.current) return;
    ...
    useEffect(() => {
        const onDocDown = (e: MouseEvent) => {
            ...
            if (t?.closest?.('[data-chat-msg-menu]')) return;
    ...
    useEffect(() => {
        ...
        client.onConnect = () => {
            ...
            client.subscribe(`/topic/group.${groupId}.chat.delete`, (message: IMessage) => {
                try {
                    const body = JSON.parse(message.body) as { messageId?: string };
                    if (body?.messageId) {
        ...
        if (kind === 'IMAGE' && m.attachment?.id) {
            return (
                <div className="space-y-1">
                    ...
                    {m.content ? (
                        <div className="text-sm whitespace-pre-wrap break-words opacity-95">
    ...
    return (
        <div className="space-y-5 max-w-4xl mx-auto min-w-0 pb-8">
            <PageHero
                badge="Чат"
                title={
                ...
                subtitle={
                    <span>
                        <Link
                            to={`/groups/${groupId}`}
                            className="text-white/95 underline decoration-white/40 hover:decoration-white font-medium"
                        >
                            Страница группы
                        </Link>
                    </span>
            ...
            >
                <Button
                    variant="secondary"
                    className="w-full justify-center sm:w-auto bg-white/95 text-indigo-900 border-white/50 dark:bg-slate-800/95 dark:text-slate-100 dark:border-slate-600"
            ...
            <Card hover={false} className="overflow-hidden shadow-xl shadow-slate-900/5 dark:shadow-black/40 ring-1 ring-slate-200/80 dark:ring-slate-700/60">
                ...
                <CardContent className="px-3 sm:px-6 pb-6 pt-5 bg-gradient-to-b from-slate-50/40 to-white dark:from-slate-950/20 dark:to-slate-950">
                    ...
                    <div
                        ...
                        className="space-y-3 sm:space-y-4 max-h-[min(62dvh,560px)] sm:max-h-[min(60vh,560px)] overflow-y-auto mb-4 pr-1 sm:pr-2 -mr-1 touch-pan-y scroll-smooth [scrollbar-width:thin]"
                    >
                        {messages.length === 0 ? (
                            ...
                            messages.map((m) => {
                                ...
                                return (
                                    <div
                                        key={m.id}
                                        className={`flex gap-2 sm:gap-3 items-end ${mine ? 'justify-end' : 'justify-start'}`}
                                    >
                                        {!mine && (
                                            <UserAvatar user={m.sender} className="h-9 w-9 sm:h-10 sm:w-10 ring-2 ring-white dark:ring-slate-800 shadow-md shrink-0" />
                                        )}
                                        <div
                                            className={`max-w-[min(92%,22rem)] sm:max-w-[76%] min-w-0 rounded-2xl px-3.5 py-2.5 sm:px-4 sm:py-3 transition-[box-shadow,transform] ${
                                                mine
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                <div
                                                    className={`text-[11px] uppercase tracking-wide flex flex-wrap gap-x-1.5 gap-y-0.5 items-baseline min-w-0 flex-1 ${
                                                    ...
                                                    {m.edited ? (
                                                        <span
                                                            className={`normal-case text-[10px] font-medium ${
                                                                mine ? 'text-amber-100/90' : 'text-slate-400 dark:text-slate-500'
                                                            }`}
                                                        >
                                                            (изменено)
                                                        </span>
                    ...
                    {mentionPick && filteredMentionUsers.length > 0 && (
                        <div className="mb-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-lg max-h-36 overflow-y-auto">
                            {filteredMentionUsers.slice(0, 8).map((u) => (
                                <button
                                    key={u.id}
                                    type="button"
                                    className="w-full text-left px-3 py-2 text-sm hover:bg-indigo-50 dark:hover:bg-slate-700 flex items-center gap-2"
                    ...
                    {showStickers && (
                        <div className="mb-3 p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/80 flex flex-wrap gap-2">
                            {STICKERS.map((s) => (
                                <button
                                    key={s}
                                    type="button"
                                    className="text-2xl p-1 rounded-lg hover:bg-white dark:hover:bg-slate-700"
                            ...
                            <button
                                type="button"
                                className="ml-auto p-1 text-slate-500"
                    ...
                    <form
                        ...
                        className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white/80 p-3 sm:p-4 shadow-sm dark:border-slate-600/70 dark:bg-slate-900/40"
                    >
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                                type="button"
                                className="inline-flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-600 p-2 hover:bg-slate-50 dark:hover:bg-slate-800"
                            ...
                            <button
                                ...
                                title={recording ? 'Отпустите для отправки' : 'Запись голоса'}
                            >
                        ...
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch sm:gap-2">
                            <Input
                                ...
                                placeholder="Сообщение… Упомяните участника: @ник"
                        ...
                        >
                            {menuKind === 'TEXT' && (
                                <button
                                    type="button"
                                    role="menuitem"
                                    className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700/80"
                                    onClick={() => openEditModal(menuMsg)}
                                >
                            ...
                            <button
                                type="button"
                                role="menuitem"
                                className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/50"
                                onClick={() => {
                                    setOpenActionMenuId(null);
            ...
            >
                <form onSubmit={submitEdit} className="p-5 sm:p-6">
                    ...
                    <textarea
                        ...
                        className="w-full min-h-[132px] rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-inner focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-400/30 dark:border-slate-600 dark:bg-slate-950/50 dark:text-slate-100"
                        maxLength={2000}
