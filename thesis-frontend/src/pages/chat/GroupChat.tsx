...
import { ArrowLeft, Image as ImageIcon, Mic, Send, Smile, Trash2, X, MoreVertical, Pencil, MessageSquare, Pin, Reply, } from 'lucide-react';
...
function shortReplyPreview(parent: ChatMessage | undefined, tr: TFunction): string {
    if (!parent)
        return tr('chat.reply.unavailable');
    const k = parent.messageKind || 'TEXT';
    if (k === 'STICKER')
        return parent.content || tr('chat.reply.sticker');
    if (k === 'IMAGE')
        return tr('chat.reply.photo');
    if (k === 'AUDIO')
        return tr('chat.reply.voice');
    if (k === 'FILE')
        return parent.attachment?.originalName || tr('chat.reply.file');
    const txt = (parent.content || '').trim();
...
const GroupChat: React.FC = () => {
    ...
    const { data: groupStats } = useQuery({
        ...
        queryFn: () => groupApi.getGroupStats(groupId!),
        enabled: !!groupId,
    });
    const canPin = !!groupStats?.canPinMessages;
    ...
    const sendMutation = useMutation({
        ...
        onError: (e: unknown) => {
            const msg = e && typeof e === 'object' && 'response' in e
                ...
                : undefined;
            toastError(msg || t('chat.sendFail'));
        },
    });
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        client.onConnect = () => {
            ...
            client.subscribe(`/topic/group.${groupId}.chat.pin`, (message: IMessage) => {
                try {
                    const body = JSON.parse(message.body) as ChatMessage;
                    replaceMessage(body);
                }
    ...
    const onPickImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            success(t('chat.photoOk'));
        }
        catch {
            toastError(t('chat.photoFail'));
        }
    };
    const stopRecording = () => {
        ...
        try {
            ...
            mr.onstop = async () => {
                ...
                try {
                    ...
                    success(t('chat.voiceOk'));
                }
                catch {
                    toastError(t('chat.voiceFail'));
                }
            };
            mr.start(200);
    ...
    const pinMutation = useMutation({
        mutationFn: ({ messageId, pinned }: {
            messageId: string;
            pinned: boolean;
        }) => chatApi.setMessagePin(messageId, pinned),
        onSuccess: (_, { pinned }) => {
            setOpenActionMenuId(null);
            success(pinned ? t('chat.pinOn') : t('chat.pinOff'));
        },
        onError: () => toastError(t('chat.pinFail')),
    });
    const deleteMutation = useMutation({
        ...
        onError: () => toastError(t('chat.deleteFail')),
    });
    const editMutation = useMutation({
        ...
        onSuccess: () => {
            setEditTarget(null);
            setEditDraft('');
            success(t('chat.updated'));
            queryClient.invalidateQueries({ queryKey: ['chat', groupId] });
        },
        onError: () => toastError(t('chat.updateFail')),
    });
    ...
    const submitEdit = (e: React.FormEvent) => {
        ...
        if (!editedText || editedText === (editTarget.content || '').trim()) {
            setEditTarget(null);
            return;
        }
        editMutation.mutate({ messageId: editTarget.id, text: editedText });
    };
    ...
    const renderMessageBody = (m: ChatMessage, mine: boolean) => {
        ...
        if (kind === 'FILE' && m.attachment?.id) {
            return (<div className="text-sm">
                    <span className="opacity-90">{t('chat.fileLabel')} {m.attachment.originalName}</span>
                </div>);
        }
    ...
    if (!group) {
        return (<div className="text-center py-12">
                <p className="text-gray-600 mb-4">{t('chat.notFound')}</p>
    ...
    return (<div className="space-y-5 max-w-4xl mx-auto min-w-0 pb-8">
            <PageHero badge={t('chat.badge')} title={<span className="break-words">
                        {t('chat.title', { name: group.name })}
                    </span>} subtitle={<span>
            ...
            <Card hover={false} className="overflow-hidden shadow-xl shadow-slate-900/5 dark:shadow-black/40 ring-1 ring-slate-200/80 dark:ring-slate-700/60">
                <CardHeader className="bg-gradient-to-r from-indigo-50/90 via-white to-violet-50/70 dark:from-slate-900 dark:via-slate-900/95 dark:to-indigo-950/50 border-b border-slate-200/70 dark:border-slate-700/80">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{t('chat.thread')}</h3>
                            <p className="text-xs text-slate-500 dark:text-slate-400">{t('chat.threadHint')}</p>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="px-3 sm:px-6 pb-6 pt-5 bg-gradient-to-b from-slate-50/40 to-white dark:from-slate-950/20 dark:to-slate-950">
                    <div className="rounded-[1.35rem] border border-slate-200/70 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-50/40 via-slate-50/80 to-slate-100/60 p-3 sm:p-4 dark:border-slate-700/60 dark:from-indigo-950/25 dark:via-slate-900/50 dark:to-slate-950/80 shadow-inner flex flex-col max-h-[min(62dvh,560px)] sm:max-h-[min(60vh,560px)] min-h-0">
                        {pinnedMessage ? (<div className="shrink-0 mb-3 rounded-xl border border-amber-200/80 bg-amber-50/95 dark:border-amber-900/50 dark:bg-amber-950/35 px-3 py-2.5 shadow-sm">
                                <p className="text-[11px] font-semibold uppercase tracking-wide text-amber-900/80 dark:text-amber-200/90 mb-1 flex items-center gap-1">
                                ...
                                <p className="text-xs text-slate-700 dark:text-slate-200 line-clamp-3">
                                    ...
                                    {shortReplyPreview(pinnedMessage, t)}
                                </p>
                            </div>) : null}
                    <div ref={messagesScrollRef} className="flex-1 min-h-0 space-y-3 sm:space-y-4 overflow-y-auto pr-1 sm:pr-2 -mr-1 touch-pan-y scroll-smooth [scrollbar-width:thin]">
                        {messages.length === 0 ? (<div className="text-center py-14 px-4">
                                <p className="text-slate-600 dark:text-slate-300 text-sm font-medium">{t('chat.empty')}</p>
            ...
            const kind = m.messageKind || 'TEXT';
            const parentReply = m.replyToId ? messages.find((x) => x.id === m.replyToId) : undefined;
            return (<div key={m.id} className={`flex gap-2 sm:gap-3 items-end ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    : 'rounded-bl-md bg-white/95 dark:bg-slate-800/90 text-slate-900 dark:text-slate-100 border border-slate-200/90 dark:border-slate-600/60 shadow-md shadow-slate-900/[0.06]'}`}>
                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                <div className={`text-[11px] uppercase tracking-wide flex flex-wrap gap-x-1.5 gap-y-0.5 items-baseline min-w-0 flex-1 ${mine ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>
                                                    {m.pinned ? (<span className="inline-flex shrink-0" aria-label={t('chat.pinnedBar')}><Pin className="h-3 w-3 opacity-90"/></span>) : null}
                                                    <span className="font-semibold normal-case tracking-normal">
                                                        {m.sender?.username ?? t('chat.member')}
                                                    </span>
                                                ...
                                                <div className="relative shrink-0 -mr-1 -mt-0.5" data-chat-msg-menu>
                                                        <button type="button" aria-expanded={openActionMenuId === m.id} aria-haspopup="menu" aria-label={t('chat.messageActions')} className={`rounded-lg p-1.5 transition-colors ${mine
                        ? 'text-white/85 hover:bg-white/15 hover:text-white'
                        : 'text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700/80'}`} onClick={(e) => {
                        e.stopPropagation();
                        if (openActionMenuId === m.id) {
                            ...
                            setOpenActionMenuId(m.id);
                                            ...
                                            {m.replyToId ? (<div className={`mb-2 rounded-lg border px-2 py-1.5 text-[11px] leading-snug ${mine
                    ? 'border-white/25 bg-black/10'
                    : 'border-slate-200 dark:border-slate-600 bg-slate-100/90 dark:bg-slate-900/40'}`}>
                                                    <Reply className="inline h-3 w-3 opacity-80 mr-0.5 align-middle"/>
                                                    <span className="font-medium">{parentReply?.sender?.username ?? '…'}</span>
                                                    <span className="opacity-70"> · </span>
                                                    <span className="opacity-95">{shortReplyPreview(parentReply, t)}</span>
                                                </div>) : null}
                    ...
                    {showStickers && (<div className="mb-3 p-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/80 flex flex-wrap gap-2">
                            ...
                            <button type="button" className="ml-auto p-1 text-slate-500" onClick={() => setShowStickers(false)} aria-label={t('chat.closeStickers')}>
                                <X className="h-5 w-5"/>
                            </button>
                        </div>)}
                    ...
                    <form onSubmit={handleSendText} className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white/80 p-3 sm:p-4 shadow-sm dark:border-slate-600/70 dark:bg-slate-900/40">
                        {replyTo ? (<div className="flex items-start justify-between gap-2 rounded-xl border border-indigo-200/90 bg-indigo-50/95 px-3 py-2 text-sm dark:border-indigo-800/80 dark:bg-indigo-950/45">
                                <div className="min-w-0 flex-1">
                                    <p className="text-indigo-800 dark:text-indigo-200 font-medium text-xs">
                                        {t('chat.replyTo')} <span className="font-semibold">{replyTo.sender?.username}</span>
                                    </p>
                                    <p className="truncate text-xs text-slate-600 dark:text-slate-400 mt-0.5">
                                        {shortReplyPreview(replyTo, t)}
                                    </p>
                                </div>
                                <button type="button" className="shrink-0 rounded-lg p-1 text-slate-500 hover:bg-slate-200/80 dark:hover:bg-slate-700" onClick={() => setReplyTo(null)} aria-label={t('chat.cancelReply')}>
                                    <X className="h-4 w-4"/>
                                </button>
                            </div>) : null}
            ...
            : 'border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`} onClick={() => (recording ? stopRecording() : startRecording())} title={recording ? t('chat.recordingRelease') : t('chat.recordVoice')}>
                            ...
                            {recording && (<span className="text-xs text-rose-600 dark:text-rose-400 font-medium">
                                    {t('chat.recordingTitle')}
                                </span>)}
                        </div>
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch sm:gap-2">
                            <Input ref={inputRef} value={text} onChange={(e) => setText(e.target.value)} placeholder={t('chat.placeholder')} className="flex-1 min-w-0 w-full" maxLength={2000}/>
            ...
            (() => {
                ...
                const menuMine = menuMsg.sender?.id === user?.id;
                return createPortal(<div role="menu" data-chat-msg-menu style={{
                        position: 'fixed',
                        top: actionMenuCoords.top,
                        left: actionMenuCoords.left,
                        zIndex: 70,
                    }} className="min-w-[12rem] overflow-hidden rounded-xl border border-slate-200/95 bg-white py-1 text-slate-800 shadow-2xl dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                            <button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700/80" onClick={() => {
                        setReplyTo(menuMsg);
                        setOpenActionMenuId(null);
                    }}>
                            ...
                            {canPin ? (<button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700/80" onClick={() => {
                        pinMutation.mutate({
                            messageId: menuMsg.id,
                            pinned: !menuMsg.pinned,
                        });
                    }} disabled={pinMutation.isPending}>
                                    <Pin className="h-4 w-4 shrink-0 opacity-80"/>
                                    {menuMsg.pinned ? t('chat.unpin') : t('chat.pin')}
                                </button>) : null}
                            {menuKind === 'TEXT' && menuMine ? (<button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700/80" onClick={() => openEditModal(menuMsg)}>
                                    <Pencil className="h-4 w-4 shrink-0 opacity-80"/>
                                    {t('chat.edit')}
                                </button>) : null}
                            {menuMine ? (<button type="button" role="menuitem" className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/50" onClick={() => {
                        setOpenActionMenuId(null);
